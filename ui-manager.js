...
export class UIManager {
    ...
    createSettingsPanel() {
        const html = `<div id="wt-settings" class="wt-settings"><div class="inline-drawer">
            <div class="inline-drawer-toggle inline-drawer-header">
                <b>🐶 World Tracker <span class="wt-version" style="cursor:default;user-select:none">v0.8.19</span></b>
    ...
    _setupTextSelection() {
        let _selBtn = null;
        let _selText = '';  // v0.8.19: 텍스트를 외부 변수로 보관 (selection 해제돼도 유지)
        let _selMesId = null;  // v0.8.19: 선택된 메시지의 인덱스 (장소 추적용)
        let _selBtnCreatedAt = 0;  // v0.8.19: 버튼 생성 시점 (500ms 보호 구간용)
        const self = this;
        const removeBtn = () => {
            if (_selBtn) { _selBtn.remove(); _selBtn = null; _selText = ''; _selMesId = null; _selBtnCreatedAt = 0; }
            clearTimeout(_selAutoCloseTimer);  // v0.8.19: 타이머도 정리
        };

        // v0.8.19: selectionchange 복귀 + 안정화 대기 + 버튼 재활용
        //   - selectionchange 발동 시 300ms 디바운스 → 드래그 완료 시점 포착
        //   - 이미 버튼 있으면 재생성 X, 텍스트/위치만 업데이트 (깜빡임 방지)
        //   - mouseup/touchend는 백업으로 유지
        ...
        const _checkSelection = () => {
            ...
            _selCheckTimer = setTimeout(() => {
                ...
                // v0.8.19: 이미 버튼 있으면 텍스트/위치만 업데이트 (재생성 X → 깜빡임 방지)
                if (_selBtn) {
                    _selText = text;
                    const mesDivU = mesTextEl.closest('.mes[mesid]');
                    _selMesId = mesDivU ? parseInt(mesDivU.getAttribute('mesid'), 10) : null;
                    // 위치 업데이트
                    const rectsU = range.getClientRects();
                    const lastRectU = rectsU && rectsU.length > 0 ? rectsU[rectsU.length - 1] : range.getBoundingClientRect();
                    const vwU = window.innerWidth;
                    const vhU = window.innerHeight;
                    const bSize = 40, gapU = 12, marginU = 8;
                    let leftU = lastRectU.right + gapU;
                    if (leftU + bSize > vwU - marginU) {
                        leftU = Math.max(marginU, Math.min(vwU - bSize - marginU, lastRectU.right - bSize));
                    }
                    let topU = lastRectU.bottom + 6;
                    if (topU + bSize > vhU - marginU) {
                        topU = Math.max(marginU, lastRectU.top - bSize - 6);
                    }
                    _selBtn[0].style.left = `${leftU}px`;
                    _selBtn[0].style.top = `${topU}px`;
                    // 자동 제거 타이머도 연장
                    clearTimeout(_selAutoCloseTimer);
                    _selAutoCloseTimer = setTimeout(removeBtn, 10000);
                    dbg(`📝 drag updated: ${text.length}c`);
                    return;
                }
                ...
                // v0.8.19: 버튼 위치 개선 — 선택 영역 "마지막 줄 우측 하단"에 정확히 배치
                //   이전 문제: 다중 줄 선택 시 rect.right는 문장 박스 전체 우측 → 화면 끝으로 튕김
                //   해결: getClientRects()로 마지막 줄 box를 정확히 얻어서 그 우측에 배치
                const btnSize = 40;
                const gap = 12;
                const margin = 8;
                const rects = range.getClientRects();
                // 마지막 줄 rect (다중 줄 선택 시 가장 아래쪽 박스)
                const lastRect = rects && rects.length > 0 ? rects[rects.length - 1] : rect;

                // 가로: 마지막 줄의 우측 끝 + 여백
                let left = lastRect.right + gap;
                if (left + btnSize > vw - margin) {
                    // 화면 우측 공간 부족 → 마지막 줄의 하단 아래로 내려서 배치
                    left = Math.max(margin, Math.min(vw - btnSize - margin, lastRect.right - btnSize));
                }

                // 세로: 마지막 줄 바로 아래 (선택 영역에 딱 붙지 않게 8px 여백)
                let top = lastRect.bottom + 6;
                if (top + btnSize > vh - margin) {
                    // 화면 하단 공간 부족 → 마지막 줄 상단 위로 배치
                    top = Math.max(margin, lastRect.top - btnSize - 6);
                }

                _selBtn = $(`<div id="wt-sel-event-btn" style="position:fixed !important;top:${top}px !important;left:${left}px !important;width:${btnSize}px !important;height:${btnSize}px !important;z-index:2147483646 !important;display:flex !important;align-items:center !important;justify-content:center !important;background:linear-gradient(135deg,#5E84E2 0%,#3D5FB0 100%);border:2px solid #fff;border-radius:50%;box-shadow:0 4px 14px rgba(94,132,226,0.5),0 2px 6px rgba(0,0,0,0.2);cursor:pointer;-webkit-tap-highlight-color:transparent;isolation:isolate;font-size:20px;user-select:none;animation:wtSelBtnPop .2s ease-out" title="이벤트로 저장">
                    📝
                </div>
                <style>@keyframes wtSelBtnPop { 0% { transform: scale(0.3); opacity: 0 } 100% { transform: scale(1); opacity: 1 } }</style>`);

                // v0.8.19: pointerdown 사용 — click/touchend 경쟁 회피 + 반응성 ↑
                ...
                const fireEvent = (ev) => {
                    ...
                    if (_btnFired) return;
                    _btnFired = true;
                    const textToSave = _selText;  // v0.8.19: 저장된 텍스트 사용 (selection 해제돼도 OK)
                    const mesIdToSave = _selMesId;  // v0.8.19: 메시지 인덱스
                ...
                // v0.8.19: body의 transform 회피를 위해 documentElement에 append
                document.documentElement.appendChild(_selBtn[0]);
                _selBtnCreatedAt = Date.now();  // v0.8.19: 생성 시점 기록 (보호 구간용)
                _selAutoCloseTimer = setTimeout(removeBtn, 10000);  // v0.8.19: 10초 타이머 (참조 저장)
            }, 150);  // 150ms 디바운스 — selectionchange 연속 발동 방지
        };

        // v0.8.19: selectionchange 메인 + mouseup/touchend 백업
        ...
        // v0.8.19: 외부 터치 감지 — 생성 직후 500ms 보호 구간 + pointerup 사용
    ...
    // v0.8.19: 드래그된 메시지의 mesid를 기반으로 "해당 시점의 장소" 추적
    ...
    // v0.8.19: 하이브리드 저장 — 50자 미만은 그대로, 이상은 AI 요약 + mood + title
    async _doSaveEvent(loc, text, mesId = null) {
        ...
        try {
            ...
            // v0.8.19: 요약 프롬프트 대폭 개선 — 구체 가이드 + 장소 컨텍스트 + 예시
...
${trimmed.substring(0, 1500)}`;
            ...
            if (result) {
                ...
                if (p?.summary) {
                    ...
                    window._wtLastLLMError = `JSON 파싱 OK, but no 'summary' field. Got: ${JSON.stringify(p).substring(0, 200)}`;
                } else {
            ...
            if (!evText) {
                ...
                evText = trimmed;  // v0.8.19: 80자 자르기 제거 — 전체 보관
    ...
    _showBottomSheet(locId) {
        ...
        if (events.length) {
            ...
            eventsHtml = recentEvents.map((ev, displayIdx) => {
                ...
                const hasDetail = ev.text && ev.text.length > 0 && ev.text !== ev.title;  // v0.8.19: 조건 완화
        ...
        bs.find('.wt-bs-tab').on('click', function(e) {
            ...
            // v0.8.19: 활성 탭 DOM 속성에 저장 (삭제 후 재렌더 시 복원용)
    ...
    // v0.8.19: 현재 활성 바텀시트 탭 반환 (events/review/community/rooms/nodemap/overview)
    _getActiveBsTab() {
        const bs = document.getElementById('wt-bottomsheet');
        if (!bs) return 'overview';
        // v0.8.19: 저장된 data 속성 우선 사용 (:visible 감지보다 안정적)
    ...
    // v0.8.19: 바텀시트 재렌더 + 이전 탭 복원 (이벤트/일정 삭제 등에서 사용)
    ...
    _updEventsList(locId) {
        ...
        recent.forEach((ev, i) => {
            ...
            const hasDetail = ev.text && ev.text.length > 0 && ev.text !== title;  // v0.8.19: 조건 완화
    ...
    async _generateCommunity(locId) {
        ...
        try {
            ...
            // v0.8.19: 그룹챗 감지 — charName에 쉼표 있거나 ctx.groupId 존재 시
            ...
            // v0.8.19: NPC 목록 나열 버그 방지 — 최대 2명만 전달 (주 캐릭터 기준으로 관련도 높은 NPC 우선)
            ...
            // v0.8.19: 현지 정보 보강 — 설정에 따라 POI 검색 실행
            ...
            if (enrichMode === 'grounding') {
                ...
                toastSuccess('⭐ Google 검색 기반 생성 중... (30~60초 소요)');
            }
...
JSON만 응답. 앞뒤에 설명·코드블록·주석 금지.`;
            ...
            // v0.8.19: 창의성 ↑ — 커뮤니티는 temperature 0.95로 다양한 톤 유도
            // v0.8.19: maxTokens도 분량에 맞춰 (기본 4096은 7~9개 생성에 부족 → 잘림)
            // v0.8.19: Gemini 2.5 Pro는 thinking이 출력 토큰 차지 → 2배 증량
            ...
            window._wtMaxTokensOverride = null;
            window._wtUseGrounding = false; // v0.8.19: grounding 플래그 원복
            // v0.8.19: 디버그 로그 모달용 저장
            ...
            if (!result) {
                ...
                if (/503|429|500|502|504|과부하|overload/i.test(err)) {
                    ...
                    const enrichOn = s?.locationEnrichment === 'grounding';
                    if (enrichOn) {
                        toastWarn(`⚠️ Grounding 응답 시간 초과 (90초). 🔍 현지 정보 보강을 🌐 무료 또는 OFF로 변경 권장`);
                    } else {
                        toastWarn(`⚠️ 응답 시간 초과. 📏 생성 분량 → 🌱 가벼움 권장 (🐛 버튼으로 로그 확인)`);
                    }
                } else if (/RP story|RP 이어쓰기|RP continuation/i.test(err)) {
                    // v0.8.19: Pro 모델이면 더 구체적인 안내
                    const m = s?.llmModel || '';
                    if (/pro/i.test(m)) {
            ...
            if (!parsed?.posts || !Array.isArray(parsed.posts)) {
                // v0.8.19: 모바일 대응 — raw response를 window에 저장해 디버그 뷰어에서 볼 수 있게
                window._wtLastRawResponse = result || '';
                window._wtLastErrorType = 'parse_failed';
                window._wtLastErrorAt = new Date().toLocaleString('ko-KR');
            ...
            for (const p of parsed.posts.filter(p => p && p.text)) {
                ...
                // v0.8.19: 답글 정제 (name/handle/avatar/text만 유지, 최대 3개)
            ...
            if (!this._commOverlayOpen) {
                ...
                // v0.8.19: 현재 활성 탭 기억 → 재렌더 후 복원 (🟢 실시간 탭에서 생성 시 탭 유지)
    ...
    // v0.8.19: 모바일용 — 마지막 LLM 응답 + 에러를 화면 내 모달로 표시
    _showDebugLogModal() {
        ...
        // v0.8.19: 현재 저장된 설정 값 확인 (키 자체는 마스킹)
        const s = extension_settings[EXTENSION_NAME] || {};
        const cfgSummary = [
    ...
    // v0.8.19: 현지 정보 보강 — Nominatim에서 주변 POI 정보 가져오기
    ...
    // v0.8.19: 이모지/멀티바이트 문자의 첫 grapheme만 추출 (아바타 overflow 방지)
    _firstGrapheme(s) {
        if (!s) return '👤';
        try {
    ...
    _renderCommunityPostCard(p) {
        ...
        // v0.8.19: 아바타 정규화 — 이모지 2~3개 겹친 거("🍯🦡", "1️⃣4️⃣1️⃣") 터지지 않도록 첫 grapheme만 사용
        const avatarChar = this._firstGrapheme(p.avatar || (p.type === 'animal' ? '🐾' : '👤'));
        // v0.8.19: 답글 렌더링 (트위터 스타일 — 왼쪽 살짝 들여쓰기 + 가는 선)
    ...
    _showNpcProfile(locId, npcName) {
        ...
        // v0.8.19: SillyTavern 모바일에서 body의 transform 때문에 position:fixed 깨짐 방지
...
CRITICAL: Start with { end with }`;
    ...
    async _generateReviews(locId, source = 'auto') {
        ...
        try {
            ...
            // v0.8.19: 생성 분량 설정에 따른 리뷰 수 (방문횟수도 여전히 약간 반영)
...
CRITICAL: Start your response with { and end with }. Nothing else.`;
            ...
            // v0.8.19: 리뷰도 분량 설정에 맞춰 토큰 한도 조정
