...
export class UIManager {
    ...
    createSettingsPanel() {
        const html = `<div id="wt-settings" class="wt-settings"><div class="inline-drawer">
            <div class="inline-drawer-toggle inline-drawer-header">
                <b>🐶 World Tracker <span class="wt-version" style="cursor:default;user-select:none">v0.8.0</span></b>
    ...
    _showBottomSheet(locId) {
        ...
        bs.find('.wt-bs-tab').on('click', function(e) {
            ...
            // v0.8.0: 활성 탭 DOM 속성에 저장 (삭제 후 재렌더 시 복원용)
    ...
    // v0.8.0: 현재 활성 바텀시트 탭 반환 (events/review/community/rooms/nodemap/overview)
    _getActiveBsTab() {
        const bs = document.getElementById('wt-bottomsheet');
        if (!bs) return 'overview';
        // v0.8.0: 저장된 data 속성 우선 사용 (:visible 감지보다 안정적)
    ...
    // v0.8.0: 바텀시트 재렌더 + 이전 탭 복원 (이벤트/일정 삭제 등에서 사용)
    ...
    async _generateCommunity(locId) {
        ...
        try {
            ...
            // v0.8.0: NPC 목록 나열 버그 방지 — 최대 2명만 전달 (주 캐릭터 기준으로 관련도 높은 NPC 우선)
            ...
            // v0.8.0: 현지 정보 보강 — 설정에 따라 POI 검색 실행
...
🎨🎨🎨 **톤 다양성 필수 — 30/40/30 비율로 섞기!** 🎨🎨🎨
**절대 모든 트윗이 감탄사로 시작하거나 ㅋㅋㅠㅠ로 도배되지 말 것!** 실제 여초 트위터는 톤이 섞여있다.

📊 **반드시 지킬 비율:**

**[30%] 담백한 관찰/일상 트윗** (ㅋㅋ/ㅠㅠ 없이도 OK)
  예시 샘플:
  - "자취하면 귀여운 주방용품만 사게 됨"
  - "오늘 노을 예뻤다."
  - "사직로 카페 자리 잡기 쉽지 않네."
  - "야간개장 예매 성공."
  포인트: 간결한 관찰, 건조한 종결, 감정 절제. 이모지·감탄사 없음.

**[40%] 약한 감성 트윗** (ㅋㅋ 1~2개, 차분한 감정)
  예시 샘플:
  - "이 동네 고양이 또 왔음 ㅋㅋ"
  - "사진 찍는데 사람 많아서 혼났다 ㅠ"
  - "오늘 여기 사람 진짜 많네 ㅋㅋㅋ"
  포인트: 적당한 감정 표현, 과하지 않은 호들갑.

**[30%] 폭발 감성 트윗** (ㅋㅋㅋㅋㅋ 5개+, 미친/실화/심장)
  예시 샘플:
  - "미친 이거 실화냐고 ㅋㅋㅋㅋㅋㅋ 나 지금 심장 해롭다ㅠㅠ"
  - "와 와 와 이거 무슨 일임??? 나 진짜 ㅋㅋㅋㅋ 말문 막힘"
  - "헐 대박 미쳤다... 눈물 흘릴뻔 ㅠㅠㅠㅠ"
  포인트: 최대 호들갑, 긴 ㅋㅋㅋㅋㅋ/ㅠㅠㅠㅠ, 감탄사 폭격.

⛔ **절대 금지:**
  - 연속된 트윗이 모두 "미친/헐/와"로 시작 (분산하기)
  - 모든 트윗에 ㅋㅋ/ㅠㅠ 들어가기 (30%는 아예 안 넣어야 함)
  - 이모티콘 남발 (절제도 힘이다)

✅ **시작 방식 다양화:**
  - 담백: "오늘", "방금", "아 근데", "여기", "오늘따라"
  - 약함: "아 진짜 ㅋㅋ", "생각해보니", "어제부터"
  - 폭발: "미친", "와 씨", "헐", "어떡해", "실화냐"
...
✏️✏️✏️ **한국어 맞춤법 주의 (언어가 한국어일 때)** ✏️✏️✏️
LLM이 자주 틀리는 맞춤법 — 아래 정확한 표기 사용:
  ✅ "어떡해" (감탄/당황, what should I do) — "어떡해 너무 귀여움", "나 어떡해ㅠㅠ"
  ❌ "어떻게 너무 귀여움" ← 틀림 (어떻게=how, 방식을 물을 때만)
  
  ✅ "안 돼" (불가능) — "이거 안 돼요", "포기가 안 돼"
  ❌ "않되" "안되" ← 틀림
  
  ✅ "됐어/됐음" (완료) — "다 됐음!"
  ❌ "됬어/됬음" ← 틀림
  
  ✅ "왠지" (why), "웬" (which) — "왠지 좋음", "웬 비둘기?"
  ❌ 둘 바꿔쓰지 말 것

  ✅ "뭐라고" (quoting) — "뭐라고 썼는지 기억 못할듯"
  ❌ "머라고"는 구어체 OK이지만 남발 금지
...
JSON만 응답. 앞뒤에 설명·코드블록·주석 금지.`;
            ...
            // v0.8.0: 창의성 ↑ — 커뮤니티는 temperature 0.95로 다양한 톤 유도
            // v0.8.0: maxTokens도 분량에 맞춰 (기본 4096은 7~9개 생성에 부족 → 잘림)
            ...
            window._wtUseGrounding = false; // v0.8.0: grounding 플래그 원복
            // v0.8.0: 디버그 로그 모달용 저장
            ...
            if (!parsed?.posts || !Array.isArray(parsed.posts)) {
                // v0.8.0: 모바일 대응 — raw response를 window에 저장해 디버그 뷰어에서 볼 수 있게
                window._wtLastRawResponse = result || '';
            ...
            for (const p of parsed.posts.filter(p => p && p.text)) {
                ...
                // v0.8.0: 답글 정제 (name/handle/avatar/text만 유지, 최대 3개)
            ...
            if (!this._commOverlayOpen) {
                ...
                // v0.8.0: 현재 활성 탭 기억 → 재렌더 후 복원 (🟢 실시간 탭에서 생성 시 탭 유지)
    ...
    // v0.8.0: 모바일용 — 마지막 LLM 응답 + 에러를 화면 내 모달로 표시
    _showDebugLogModal() {
    ...
    // v0.8.0: 현지 정보 보강 — Nominatim에서 주변 POI 정보 가져오기
    ...
    // v0.8.0: 이모지/멀티바이트 문자의 첫 grapheme만 추출 (아바타 overflow 방지)
    _firstGrapheme(s) {
        if (!s) return '👤';
        try {
    ...
    _renderCommunityPostCard(p) {
        ...
        // v0.8.0: 아바타 정규화 — 이모지 2~3개 겹친 거("🍯🦡", "1️⃣4️⃣1️⃣") 터지지 않도록 첫 grapheme만 사용
        const avatarChar = this._firstGrapheme(p.avatar || (p.type === 'animal' ? '🐾' : '👤'));
        // v0.8.0: 답글 렌더링 (트위터 스타일 — 왼쪽 살짝 들여쓰기 + 가는 선)
    ...
    _showNpcProfile(locId, npcName) {
        ...
        // v0.8.0: SillyTavern 모바일에서 body의 transform 때문에 position:fixed 깨짐 방지
...
CRITICAL: Start with { end with }`;
    ...
    async _generateReviews(locId, source = 'auto') {
        ...
        try {
            ...
            // v0.8.0: 생성 분량 설정에 따른 리뷰 수 (방문횟수도 여전히 약간 반영)
