...
export class UIManager {
    ...
    createSettingsPanel() {
        const html = `<div id="wt-settings" class="wt-settings"><div class="inline-drawer">
            <div class="inline-drawer-toggle inline-drawer-header">
                <b>🐶 World Tracker <span class="wt-version" style="cursor:default;user-select:none">v0.8.26</span></b>
    ...
    _setupTextSelection() {
        let _selBtn = null;
        let _selText = '';  // v0.8.26: 텍스트를 외부 변수로 보관 (selection 해제돼도 유지)
        let _selMesId = null;  // v0.8.26: 선택된 메시지의 인덱스 (장소 추적용)
        let _selBtnCreatedAt = 0;  // v0.8.26: 버튼 생성 시점 (500ms 보호 구간용)
        const self = this;
        const removeBtn = () => {
            if (_selBtn) { _selBtn.remove(); _selBtn = null; _selText = ''; _selMesId = null; _selBtnCreatedAt = 0; }
            clearTimeout(_selAutoCloseTimer);  // v0.8.26: 타이머도 정리
        };

        // v0.8.26: 드래그 감지 완전 리팩토링
        ...
        const _checkSelection = () => {
            ...
            _selCheckTimer = setTimeout(() => {
                ...
                // v0.8.26: 빈 selection이어도 기존 버튼 유지 (손 뗐을 때 selection 사라지는 경우 대비)
                ...
                // v0.8.26: 버튼 있을 때 "더 긴 selection"이면 텍스트 업데이트, 아니면 유지
                if (_selBtn) {
                ...
                // v0.8.26: 위치 고정 — 우측 상단 (SillyTavern 상단 툴바 아래 여유 공간)
                ...
                // v0.8.26: pointerdown 사용 — click/touchend 경쟁 회피 + 반응성 ↑
                ...
                const fireEvent = (ev) => {
                    ...
                    if (_btnFired) return;
                    _btnFired = true;
                    const textToSave = _selText;  // v0.8.26: 저장된 텍스트 사용 (selection 해제돼도 OK)
                    const mesIdToSave = _selMesId;  // v0.8.26: 메시지 인덱스
                ...
                // v0.8.26: body의 transform 회피를 위해 documentElement에 append
                document.documentElement.appendChild(_selBtn[0]);
                _selBtnCreatedAt = Date.now();  // v0.8.26: 생성 시점 기록 (보호 구간용)
                _selAutoCloseTimer = setTimeout(removeBtn, 10000);  // v0.8.26: 10초 타이머 (참조 저장)
            }, 150);  // 150ms 디바운스 — selectionchange 연속 발동 방지
        };

        // v0.8.26: selectionchange 메인 + mouseup/touchend 백업
        ...
        // v0.8.26: 외부 터치 감지 — 생성 직후 500ms 보호 구간 + pointerup 사용
    ...
    // v0.8.26: 드래그된 메시지의 mesid를 기반으로 "해당 시점의 장소" 추적
    ...
    // v0.8.26: 하이브리드 저장 — 50자 미만은 그대로, 이상은 AI 요약 + mood + title
    async _doSaveEvent(loc, text, mesId = null) {
        ...
        try {
            ...
            // v0.8.26: 요약 프롬프트 대폭 개선 — 구체 가이드 + 장소 컨텍스트 + 예시
...
${trimmed.substring(0, 1500)}`;
            ...
            if (!evText) {
                ...
                evText = trimmed;  // v0.8.26: 80자 자르기 제거 — 전체 보관
    ...
    _showBottomSheet(locId) {
        ...
        if (events.length) {
            ...
            eventsHtml = recentEvents.map((ev, displayIdx) => {
                ...
                const hasDetail = ev.text && ev.text.length > 0 && ev.text !== ev.title;  // v0.8.26: 조건 완화
                ...
                // v0.8.26: 날짜 수정 UI — date-view/date-edit + ✏️ 버튼 (LLM이 잘못 저장한 rpDate 수동 정정)
        ...
        bs.find('.wt-bs-tab').on('click', function(e) {
            ...
            // v0.8.26: 활성 탭 DOM 속성에 저장 (삭제 후 재렌더 시 복원용)
        ...
        // v0.8.26: 날짜 수정 — 날짜 탭 → input으로 전환 → Enter/blur로 저장
        ...
        // v0.8.26: 장소 방문일자 (첫/최근) 수기 수정 — rpFirstVisited / rpLastVisited
    ...
    // v0.8.26: 현재 활성 바텀시트 탭 반환 (events/review/community/rooms/nodemap/overview)
    _getActiveBsTab() {
        const bs = document.getElementById('wt-bottomsheet');
        if (!bs) return 'overview';
        // v0.8.26: 저장된 data 속성 우선 사용 (:visible 감지보다 안정적)
    ...
    // v0.8.26: 바텀시트 재렌더 + 이전 탭 복원 (이벤트/일정 삭제 등에서 사용)
    ...
    _showTimelineBS() {
        ...
        for (const [dayKey, group] of sorted) {
            ...
            // 날짜 헤더
            // v0.8.26: 날짜 부분 탭하면 group 전체 mov/events의 rpDate 일괄 수정
            // v0.8.26: 그 날 전체 삭제 🗑️ 버튼 추가
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                // v0.8.26: 개별 이동 삭제 버튼 (현재 위치는 삭제 불가)
        ...
        if (allPlans.length) {
            planHtml = `<div style="margin:0 16px;padding-top:10px;border-top:1.5px dashed #E0DDD5">
                ...
                ${allPlans.map(p => `<div style="display:flex;gap:10px;padding:6px 0;border-bottom:1px solid #F5F5F5;align-items:flex-start">
                    ...
                    <span class="wt-plan-del" data-loc-id="${p.locId}" data-ev-ts="${p.timestamp || 0}" style="cursor:pointer;color:#B8A89A;font-size:13px;padding:4px 8px;border-radius:6px;touch-action:manipulation;flex-shrink:0" title="일정 삭제">✕</span>
                </div>`).join('')}
            </div>`;
        }
        ...
        // v0.8.26: 타임라인 날짜 헤더 수정 — 그룹 내 모든 mov.rpDate 일괄 변경 + 그 날의 이벤트 rpDate도 같이
        const self2 = this;
        ...
        $(document).on('blur.wtTlDe', '.wt-tl-dedit', async function() {
            ...
            if (newDate) {
                // v0.8.26: 실제 API — lm.movements 배열 직접 사용 + db.put으로 저장
        ...
        // v0.8.26: 타임라인 삭제 — 개별 이동 / 날짜 그룹 / 전체 초기화
        ...
        $(document).on('click.wtPlanDelTl touchend.wtPlanDelTl', '.wt-plan-del', async function(e) {
            e.preventDefault(); e.stopPropagation();
            if (window._wtTapFireLock) return;
            window._wtTapFireLock = true;
            setTimeout(() => window._wtTapFireLock = false, 600);
    ...
    _updEventsList(locId) {
        ...
        recent.forEach((ev, i) => {
            ...
            const hasDetail = ev.text && ev.text.length > 0 && ev.text !== title;  // v0.8.26: 조건 완화
    ...
    async _generateCommunity(locId) {
        ...
        try {
            ...
            // v0.8.26: 그룹챗 감지 — charName에 쉼표 있거나 ctx.groupId 존재 시
            ...
            // v0.8.26: NPC 목록 나열 버그 방지 — 최대 2명만 전달 (주 캐릭터 기준으로 관련도 높은 NPC 우선)
            ...
            // v0.8.26: 현지 정보 보강 — 설정에 따라 POI 검색 실행
...
JSON만 응답. 앞뒤에 설명·코드블록·주석 금지.`;
            ...
            // v0.8.26: 창의성 ↑ — 커뮤니티는 temperature 0.95로 다양한 톤 유도
            // v0.8.26: maxTokens도 분량에 맞춰 (기본 4096은 7~9개 생성에 부족 → 잘림)
            // v0.8.26: Gemini 2.5 Pro는 thinking이 출력 토큰 차지 → 2배 증량
            ...
            window._wtMaxTokensOverride = null;
            window._wtUseGrounding = false; // v0.8.26: grounding 플래그 원복
            // v0.8.26: 디버그 로그 모달용 저장
            ...
            if (!parsed?.posts || !Array.isArray(parsed.posts)) {
                // v0.8.26: 모바일 대응 — raw response를 window에 저장해 디버그 뷰어에서 볼 수 있게
                window._wtLastRawResponse = result || '';
                window._wtLastErrorType = 'parse_failed';
                window._wtLastErrorAt = new Date().toLocaleString('ko-KR');
            ...
            for (const p of parsed.posts.filter(p => p && p.text)) {
                ...
                // v0.8.26: 답글 정제 (name/handle/avatar/text만 유지, 최대 3개)
            ...
            if (!this._commOverlayOpen) {
                ...
                // v0.8.26: 현재 활성 탭 기억 → 재렌더 후 복원 (🟢 실시간 탭에서 생성 시 탭 유지)
    ...
    // v0.8.26: 모바일용 — 마지막 LLM 응답 + 에러를 화면 내 모달로 표시
    _showDebugLogModal() {
        ...
        // v0.8.26: 현재 저장된 설정 값 확인 (키 자체는 마스킹)
        const s = extension_settings[EXTENSION_NAME] || {};
        const cfgSummary = [
    ...
    // v0.8.26: 현지 정보 보강 — Nominatim에서 주변 POI 정보 가져오기
    ...
    // v0.8.26: 이모지/멀티바이트 문자의 첫 grapheme만 추출 (아바타 overflow 방지)
    _firstGrapheme(s) {
        if (!s) return '👤';
        try {
    ...
    _renderCommunityPostCard(p) {
        ...
        // v0.8.26: 아바타 정규화 — 이모지 2~3개 겹친 거("🍯🦡", "1️⃣4️⃣1️⃣") 터지지 않도록 첫 grapheme만 사용
        const avatarChar = this._firstGrapheme(p.avatar || (p.type === 'animal' ? '🐾' : '👤'));
        // v0.8.26: 답글 렌더링 (트위터 스타일 — 왼쪽 살짝 들여쓰기 + 가는 선)
    ...
    _showNpcProfile(locId, npcName) {
        ...
        // v0.8.26: SillyTavern 모바일에서 body의 transform 때문에 position:fixed 깨짐 방지
        // → documentElement(html)에 직접 append해서 모든 탭/오버레이 위에 확실히 표시
        document.documentElement.appendChild(overlay[0]);
        // v0.8.26: Gemini 조언 반영 — @keyframes + 애니메이션 끝 후 transform 완전 제거
        ...
        requestAnimationFrame(() => {
            ...
            // v0.8.26: 애니메이션 끝나면 inline 스타일까지 완전 제거 → 100% 선명
            const onAnimEnd = () => {
                sheet.classList.remove('wt-npc-sheet-entering');
                sheet.classList.add('wt-npc-sheet-done');
                // 안전장치: inline transform/animation 속성도 직접 클리어
                sheet.style.animation = 'none';
                sheet.style.transform = 'translateZ(0)';  // GPU 유지, 2D 변환 X
...
CRITICAL: Start with { end with }`;
    ...
    async _generateReviews(locId, source = 'auto') {
        ...
        try {
            ...
            // v0.8.26: 생성 분량 설정에 따른 리뷰 수 (방문횟수도 여전히 약간 반영)
...
CRITICAL: Start your response with { and end with }. Nothing else.`;
            ...
            // v0.8.26: 리뷰도 분량 설정에 맞춰 토큰 한도 조정
