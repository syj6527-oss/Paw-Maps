...
export class UIManager {
    ...
    createSettingsPanel() {
        const html = `<div id="wt-settings" class="wt-settings"><div class="inline-drawer">
            ...
            <div class="inline-drawer-content">
                ...
                <div class="wt-s-row"><label>💭 기억</label>
                    <select id="wt-s-mem" class="text_pole wt-select"><option value="natural">🌿 자연스러운</option><option value="perfect">💎 완벽한</option></select>
                </div>
                <div class="wt-divider"></div>
    ...
    registerWandButton() {
        try {
            const btn = document.createElement('div'); btn.id = 'wt-wand-btn';
            btn.className = 'list-group-item flex-container flexGap5';
            btn.innerHTML = '<span>🗺️</span> World Tracker';
            btn.addEventListener('click', () => this.togglePanel());
            const menu = document.getElementById('extensionsMenu');
            if (menu) menu.appendChild(btn);
        } catch (e) { console.warn(`[${EXTENSION_NAME}] Wand btn:`, e.message); }
    }
    ...
    createSidePanel() {
        ...
        <div id="wt-panel" class="wt-panel">
            ...
            <div class="wt-panel-body">
                ...
                <!-- 접이식 장소 추가 -->
                <div class="wt-add-toggle" id="wt-add-toggle">✚ 장소 추가 ▾</div>
    ...
    _bindPanel() {
        ...
        $('#wt-add-toggle').on('click', () => { $('#wt-add-form').slideToggle(200); });
    ...
    async refresh() {
        ...
        this._updDual(); this._updLocList(); this._updMoveList(); this._updPrompt();
    }
    ...
    _updPrompt() {
        const s = extension_settings[EXTENSION_NAME];
        if (s?.aiInjection && this.pi) { const t = this.pi.generate(); if (t) { $('#wt-prompt-text').text(t); $('#wt-prompt-preview').slideDown(150); return; } }
    ...
    _updLocList() {
        ...
        for (const loc of sorted) {
            ...
            const mk = isU&&isC?' 👤🎭':isU?' 👤':isC?' 🎭':'';
            const item = $(`<div class="wt-loc-item ${isU||isC?'wt-loc-active':''}" data-id="${loc.id}">
                <div class="wt-loc-dot" style="background:${loc.color}"></div>
                <div class="wt-loc-info"><div class="wt-loc-name">${loc.name}${mk}</div>${loc.memo?`<div class="wt-loc-memo">${loc.memo}</div>`:''}</div>
    ...
    _updMoveList() {
        ...
        for (const m of [...this.lm.movements].sort((a,b)=>b.timestamp-a.timestamp).slice(0,20)) {
            ...
            list.append(`<div class="wt-mv-item"><span class="wt-mv-icon">${icon}</span><span class="wt-mv-time">${time}</span><span class="wt-mv-from">${f.name}</span><span class="wt-mv-arrow">→</span><span class="wt-mv-to">${t.name}</span>${dist}</div>`);
        }
    }
    ...
    async _addLoc() {
        ...
        try {
            ...
            if (loc) { toastr.success(`"${name}" 추가! 🗺️`); $('#wt-input-name,#wt-input-aliases,#wt-input-memo').val(''); $('#wt-add-form').slideUp(200); this.refresh(); }
    ...
    showNewPlaceToast(name, source='char') {
        ...
        if (sim.length) {
            for (const loc of sim) {
                ...
                btn.on('click', async () => {
                    ...
                    if (source==='both') { await this.lm.moveTo(loc.id,'user'); await this.lm.moveTo(loc.id,'char'); }
                    else await this.lm.moveTo(loc.id, source);
        ...
        $('#wt-npt-add').off('click').on('click', async () => {
            ...
            if (loc) {
                ...
                toastr.success(`"${name}" 등록! 🗺️`); this.pi?.inject(); if (this.panelVisible) this.refresh();
            }
            $('#wt-npt').fadeOut(200);
        });
        $('#wt-npt').fadeIn(300);
    ...
    _findSim(name) {
        ...
        const mg = catGroups.filter(g => g.some(w => lo.includes(w)));
        if (!mg.length) return [];
        const r = [];
        for (const loc of this.lm.locations) {
            const ns = [loc.name.toLowerCase(), ...(loc.aliases||[]).map(a=>a.toLowerCase())];
            for (const g of mg) { if (ns.some(n => g.some(w => n.includes(w)))) { r.push(loc); break; } }
        }
        return r.slice(0,3);
    }
