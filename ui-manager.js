    ...
    createSettingsPanel() {
        const html = `<div id="wt-settings" class="wt-settings"><div class="inline-drawer">
            <div class="inline-drawer-toggle inline-drawer-header">
                <b>🐶 World Tracker <span class="wt-version" style="cursor:default;user-select:none">v0.6.9</span></b>
...
최근 이곳 사건: ${evSummary}
${langInst}
${recentChat ? `\n[최근 RP 맥락 (분위기 참고만, 캐릭터·인물명 직접 언급 금지)]:\n${recentChat.substring(0, 500)}\n` : ''}
...
**전부 익명 유저/동물만!** 등록된 캐릭터나 NPC는 **절대 등장 금지** (이름 언급도 X).

구성:
- **익명 사람 유저 3~4개** — 이곳 주민/방문자/지나가는 사람 (이름 없음)
- **익명 동물 1~2개** — 길고양이, 들개, 까마귀, 비둘기, 근처 가게 고양이, 공원 청설모 등 장소에 어울리는 동물. type:"animal"
...
JSON만 응답:
{"posts":[
  {"name":"사막여우42","handle":"@desert_fox42","avatar":"🦊","type":"anon","mood":"chill","moodLabel":"😌 나른","text":"미친 해 왤케 쨍쨍함... 나 지금 녹음ㅠㅠ 낮잠 각인데<br><img src='https://image.pollinations.ai/prompt/desert%20afternoon%20sunlight%20peaceful%20cinematic?nologo=true&width=512&height=340' style='width:100%;border-radius:10px;margin-top:8px'>","likes":4,"replies":[{"name":"나른한사람","handle":"@sleepy_one","avatar":"😌","text":"ㄹㅇ 자고싶다ㅠㅠㅠ"},{"name":"익명","handle":"@anon2","avatar":"👤","text":"부럽다 난 출근함..."}]},
  ...
  {"name":"야행성올빼미","handle":"@night_owl","avatar":"🌙","type":"anon","mood":"sleepy","moodLabel":"😮‍💨 피곤","text":"새벽 3시인데 잠 왜 안옴... 나만 이럼?<br><img src='https://image.pollinations.ai/prompt/night%20window%20desert%20stars%20lonely%20atmosphere?nologo=true&width=512&height=340' style='width:100%;border-radius:10px;margin-top:8px'>","likes":7,"replies":[]},
  {"name":"익명","handle":"@anon_local","avatar":"👤","type":"anon","mood":"excited","moodLabel":"👀 관찰","text":"방금 큰길에 검은 SUV 대여섯대 지나감 이거 뭐임ㅋㅋ 다들 봤어요? #속보","likes":15,"replies":[{"name":"궁금맨","handle":"@curious_0","avatar":"👀","text":"어디쯤이요??"},{"name":"소문쟁이","handle":"@gossip_lord","avatar":"💭","text":"헐 어제도 지나갔다는데... 소름"}]},
...
JSON만 응답. 앞뒤에 설명·코드블록·주석 금지.`;
            ...
            for (const p of parsed.posts.filter(p => p && p.text)) {
                ...
                // v0.6.9: 답글 정제 (name/handle/avatar/text만 유지, 최대 3개)
            ...
            if (!this._commOverlayOpen) {
                ...
                // v0.6.9: 현재 활성 탭 기억 → 재렌더 후 복원 (🟢 실시간 탭에서 생성 시 탭 유지)
    ...
    // v0.6.9: 이모지/멀티바이트 문자의 첫 grapheme만 추출 (아바타 overflow 방지)
    _firstGrapheme(s) {
        if (!s) return '👤';
        try {
    ...
    _renderCommunityPostCard(p) {
        ...
        // v0.6.9: 아바타 정규화 — 이모지 2~3개 겹친 거("🍯🦡", "1️⃣4️⃣1️⃣") 터지지 않도록 첫 grapheme만 사용
        const avatarChar = this._firstGrapheme(p.avatar || (p.type === 'animal' ? '🐾' : '👤'));
        // v0.6.9: 답글 렌더링 (트위터 스타일 — 왼쪽 살짝 들여쓰기 + 가는 선)
