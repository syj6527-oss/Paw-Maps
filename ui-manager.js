    ...
    createSettingsPanel() {
        const html = `<div id="wt-settings" class="wt-settings"><div class="inline-drawer">
            <div class="inline-drawer-toggle inline-drawer-header">
                <b>🐾 Paw Map <span class="wt-version" style="cursor:default;user-select:none">v0.9.18</span></b>
    ...
    _enhancePollinationsImages(html) {
        ...
        return html.replace(/<img\b[^>]*\bsrc=(['"])(https?:\/\/image\.pollinations\.ai\/prompt\/[^'"]+)\1[^>]*>/gi, (full, q, url) => {
            try {
                ...
                // onerror는 내가 만든 안전한 코드 (LLM 입력 아님) — sanitize 이후라 유지됨
                return `<img src='${newUrl}' loading='lazy' referrerpolicy='no-referrer' style='${style}' onerror="this.style.display='none'">`;
            } catch (_) { return full; }
        });
    }
