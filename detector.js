...
export class LocationDetector {
    constructor(lm) {
        ...
        this.presKw = ['에서 앉', '에서 서 있', '에 앉아', '에 서서', '안에 있', '안에서', '앞에 서', '문간에', '입구에'];

        this.futureKw = ['갈래', '갈까', '가자', '가볼까', '어때', '가고 싶', '가보자', '갈 거', '갈게', 'shall we', "let's go", 'want to go', 'how about'];

        // ===== 경유지 필터 (장소로 안 잡는 단어) =====
        this.transitKo = [
            '복도', '계단', '통로', '현관', '로비', '엘리베이터', '에스컬레이터',
            '출입구', '입구', '출구', '문', '문앞', '문간', '현관문',
            '골목', '길', '건널목', '횡단보도', '인도', '보도',
            '주차장', '주차', '차도', '도로',
        ];

        this.transitEn = [
            'corridor', 'hallway', 'staircase', 'stairs', 'stairwell',
            'elevator', 'escalator', 'passage', 'passageway',
            'entrance', 'exit', 'doorway', 'door', 'gate',
            'sidewalk', 'pathway', 'driveway', 'alley',
            'parking', 'parking lot',
        ];
