// 🗺️ RP World Tracker — map-renderer.js (Single Scene + Mobile)
...
export class MapRenderer {
    ...
    render() {
        ...
        for (const m of movements) {
            ...
            if (!f||!t) continue;
            const k=`${m.fromId}-${m.toId}`; if(drawn.has(k))continue; drawn.add(k);
        ...
        for (const loc of locations) {
            ...
            const circle = this._el('circle',{
                r:isCur?20:15, fill:loc.color,
                class:isCur?'wt-node-circle wt-node-current':'wt-node-circle',
                stroke:isCur?'#775537':'#9e8e7e','stroke-width':isCur?3:1.5,
            });
            if(isCur) circle.setAttribute('filter','url(#wt-glow)');
            g.appendChild(circle);

            if(loc.visitCount>0) g.appendChild(this._el('text',{class:'wt-visit-badge',y:4},loc.visitCount));
            g.appendChild(this._el('text',{class:'wt-location-label',y:isCur?34:30},loc.name));
            if(isCur) g.appendChild(this._el('text',{class:'wt-footprint-marker',y:-28},'👣'));
    ...
    _move(e) {
        ...
        const loc=this.lm.locations.find(l=>l.id===this.dragState.id); if(!loc)return;
        loc.x=Math.round(this.dragState.ox+dx); loc.y=Math.round(this.dragState.oy+dy);
        this.render();
    }
    async _up() {
